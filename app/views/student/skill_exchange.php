...
<div class="container" style="padding-bottom: 4rem;">
    ...
    <div style="margin-bottom: 2rem; display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 1rem;">
        <div>
            <h1 style="font-size: 2rem; font-weight: 800; margin-bottom: 0.5rem;">Pertukaran Keterampilan</h1>
            <p style="color: var(--text-muted);">Bagikan keterampilanmu dan temukan partner belajar sesama siswa.</p>
        </div>
        <button onclick="document.getElementById('modal-add-skill').style.display='flex'" class="btn-primary" style="display: inline-flex; align-items: center; gap: 0.5rem;">
    ...
    <!-- ═══ TABS ════════════════════════════════════════ -->
    <div style="display: flex; gap: 0.5rem; margin-bottom: 2rem; border-bottom: 1px solid var(--border-color); flex-wrap: wrap;">
        ...
        <button class="tab-btn" onclick="switchTab('tab-mine')">
            ...
            <?php if (!empty($data['my_skills'])): ?>
                <span style="background: var(--primary-color); color: white; border-radius: 99px; padding: 1px 8px; font-size: 0.75rem;"><?= count($data['my_skills']); ?></span>
            <?php endif; ?>
        </button>
        <button class="tab-btn" onclick="switchTab('tab-requests')">
            ...
            <?php if (!empty($pending)): ?>
                <span style="background: #ef4444; color: white; border-radius: 99px; padding: 1px 8px; font-size: 0.75rem;"><?= count($pending); ?></span>
            <?php endif; ?>
        </button>
    ...
    <div id="tab-browse" class="tab-content">
        <?php if (empty($data['listings'])): ?>
            <div class="glass-card" style="text-align: center; padding: 3rem;">
                <i class="fa-solid fa-handshake" style="font-size: 3rem; color: var(--text-muted); margin-bottom: 1rem;"></i>
        ...
        <?php else: ?>
            ...
            <div id="skill-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 1.25rem;">
                <?php foreach ($data['listings'] as $listing): ?>
                    <a href="<?= BASEURL; ?>/student/skill_detail/<?= $listing['id']; ?>" class="skill-card" data-skill-id="<?= $listing['skill_id'] ?? ''; ?>" style="text-decoration: none; color: inherit;">
                        <div class="glass-card" style="margin-bottom: 0; height: 100%; transition: transform 0.2s, box-shadow 0.2s; cursor: pointer;">
                            <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 1rem;">
                                <span class="badge badge-primary" style="font-size: 0.8rem;"><?= htmlspecialchars($listing['skill_name']); ?></span>
                                <?php
                                ...
                                ?>
                                <span style="background: <?= $lc; ?>22; color: <?= $lc; ?>; border: 1px solid <?= $lc; ?>44; padding: 2px 10px; border-radius: 99px; font-size: 0.75rem; font-weight: 700;">
                                    <?= $ll; ?>
                                </span>
                            </div>

                            <div style="display: flex; align-items: center; gap: 0.75rem; margin-bottom: 1rem;">
                                <div style="width: 2.75rem; height: 2.75rem; border-radius: 50%; background: linear-gradient(135deg, var(--primary-color), #6d28d9); display: flex; align-items: center; justify-content: center; font-weight: 800; color: white; font-size: 1rem; flex-shrink: 0;">
                                    <?= strtoupper(mb_substr($listing['full_name'], 0, 1)); ?>
                                </div>
                                <div>
                                    <p style="font-weight: 600; font-size: 0.9rem;"><?= htmlspecialchars($listing['full_name']); ?></p>
                                    <p style="color: var(--text-muted); font-size: 0.8rem;">@<?= htmlspecialchars($listing['username']); ?></p>
                                </div>
                            </div>

                            <?php if ($listing['description']): ?>
                                <p style="color: var(--text-muted); font-size: 0.9rem; line-height: 1.5; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden;">
    ...
    <div id="tab-mine" class="tab-content" style="display: none;">
        <?php if (empty($data['my_skills'])): ?>
            <div class="glass-card" style="text-align: center; padding: 3rem;">
                <i class="fa-solid fa-star-of-life" style="font-size: 3rem; color: var(--text-muted); margin-bottom: 1rem;"></i>
                <h3 style="margin-bottom: 0.5rem;">Belum ada keterampilan</h3>
                <p style="color: var(--text-muted); margin-bottom: 1.5rem;">Daftarkan keterampilan yang kamu miliki untuk dilihat siswa lain.</p>
                    ...
                    <div class="glass-card" style="margin-bottom: 0; position: relative;">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem;">
                            ...
                            <a href="<?= BASEURL; ?>/student/delete_skill/<?= $ms['id']; ?>"
                               onclick="return confirm('Hapus keterampilan ini?')"
                               class="btn-small btn-danger" style="font-size: 0.8rem;">
                        ...
                        ?>
                        <span style="background: <?= $lc2; ?>22; color: <?= $lc2; ?>; border: 1px solid <?= $lc2; ?>44; padding: 2px 10px; border-radius: 99px; font-size: 0.75rem; font-weight: 700; display: inline-block; margin-bottom: 0.75rem;">
                            <?= $ll2; ?>
                        </span>
                        <p style="color: var(--text-muted); font-size: 0.9rem;"><?= htmlspecialchars($ms['description'] ?? '-'); ?></p>
                        <p style="font-size: 0.8rem; color: var(--text-muted); margin-top: 0.75rem;"><i class="fa-solid fa-calendar-alt"></i> <?= date('d M Y', strtotime($ms['created_at'])); ?></p>
                    </div>
                <?php endforeach; ?>
            </div>
        <?php endif; ?>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                ...
                                <span style="color: var(--text-muted); font-size: 0.8rem;">@<?= htmlspecialchars($req['requester_username']); ?></span>
                            </td>
                            <td>
                                <?= htmlspecialchars($req['skill_name']); ?><br>
                                <span style="color: var(--text-muted); font-size: 0.8rem;"><?= $levelLabels[$req['level']] ?? $req['level']; ?></span>
                            </td>
                            <td style="max-width: 200px; color: var(--text-muted); font-size: 0.9rem;"><?= htmlspecialchars($req['message'] ?: '-'); ?></td>
                            <td>
                                ...
                                <?php else: ?>
                                    <span class="badge" style="background: rgba(239,68,68,0.15); color: #f87171; border: 1px solid rgba(239,68,68,0.3);">Ditolak</span>
                                <?php endif; ?>
                            </td>
                            <td>
                                <?php if ($req['status'] === 'pending'): ?>
                                <div style="display: flex; gap: 0.5rem;">
                                    <form action="<?= BASEURL; ?>/student/respond_exchange" method="post">
                                        ...
                                        <button type="submit" class="btn-small btn-success"><i class="fa-solid fa-check"></i> Terima</button>
                                    </form>
                                    <form action="<?= BASEURL; ?>/student/respond_exchange" method="post">
                                        <input type="hidden" name="csrf_token" value="<?= $data['csrf_token']; ?>">
                                        <input type="hidden" name="id" value="<?= $req['id']; ?>">
                                        <input type="hidden" name="action" value="reject">
                                        <button type="submit" class="btn-small btn-danger"><i class="fa-solid fa-xmark"></i> Tolak</button>
                                    </form>
                                </div>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                ...
                                <span style="color: var(--text-muted); font-size: 0.8rem;">@<?= htmlspecialchars($req['provider_username']); ?></span>
                            </td>
                            ...
                            <td style="color: var(--text-muted); font-size: 0.85rem;"><?= date('d M Y', strtotime($req['created_at'])); ?></td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
...
<!-- ═══ MODAL: ADD SKILL ═══════════════════════════════════════════════════════ -->
<div id="modal-add-skill" style="display: none; position: fixed; inset: 0; background: rgba(0,0,0,0.7); z-index: 5000; align-items: center; justify-content: center; padding: 1rem;">
    <div class="glass-card" style="width: 100%; max-width: 480px; margin: 0; position: relative;">
        ...
        <p style="color: var(--text-muted); margin-bottom: 2rem; font-size: 0.95rem;">Beritahu teman-teman tentang keahlian yang kamu punya!</p>
        ...
        <form action="<?= BASEURL; ?>/student/add_skill" method="post" class="zenith-form">
            ...
            <div class="zenith-group">
                ...
                <div class="zenith-input-wrapper">
                    <textarea name="description" rows="3" placeholder="Ceritakan singkat kemampuanmu di bidang ini..."></textarea>
                </div>
            </div>
...
.tab-btn {
    ...
    transition: all 0.2s; display: inline-flex; align-items: center; gap: 0.5rem; font-size: 0.9rem;
}
.tab-btn:hover { color: var(--text-main); }
.tab-active { color: var(--primary-color) !important; border-bottom-color: var(--primary-color) !important; }
.filter-btn {
    background: rgba(255,255,255,0.05); border: 1px solid var(--border-color);
    color: var(--text-muted); padding: 0.35rem 1rem; border-radius: 99px; cursor: pointer;
    font-size: 0.85rem; font-weight: 600; transition: all 0.2s;
}
.filter-btn:hover, .filter-active {
    background: var(--primary-color); color: white !important; border-color: var(--primary-color);
}
.skill-card:hover .glass-card { transform: translateY(-4px); box-shadow: 0 12px 24px rgba(0,0,0,0.3); }
</style>
