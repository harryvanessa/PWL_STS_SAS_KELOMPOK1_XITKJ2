...
<div class="container" style="padding-top: 1rem;">
    <div class="chat-wrapper">
        ...
        <div class="chat-header-box">
            <div>
                ...
                <p class="text-sm text-muted" style="margin-top: 0.25rem;">
                    <?= date('d M Y, H:i', strtotime($data['session']['session_date'])); ?> · dengan
                    @<?= htmlspecialchars($data['partner_name']); ?>
                </p>
            </div>
            <div style="display: flex; align-items: center; gap: 0.75rem;">
                <?php if (!empty($data['session']['meeting_link']) && $data['session']['status'] == 'confirmed'): ?>
                    <a href="<?= htmlspecialchars($data['session']['meeting_link']); ?>" target="_blank"
                        class="btn-small btn-success" style="white-space: nowrap;">
        ...
        <form action="<?= BASEURL; ?>/chat/send" method="post" class="chat-input-form">
            ...
            <textarea name="message" class="form-control" placeholder="Ketik pesan diskusi Anda..." rows="2"
                required></textarea>
            <button type="submit" class="btn-primary btn-block"
                style="border-radius: 0.75rem; padding: 0.75rem 1.25rem; width: auto;">
